...
subdf <- subset(df, Scene == "kitchen")
kitchen_dists <- as.matrix(Levenshtein_Dist(subdf$Item))
runs <- 500
...
features <- subdf[, c("ConsistencyRating", "InconsistencyRating_Bedroom",
...
pareto_set_E_ALL <- bicriterion_anticlustering( # E_ALL
  ...
  dispersion_distances = kitchen_dists
)

diversities_pareto1 <- apply(pareto_set_E_ALL, 1, diversity_objective, x = kplus_features)
dispersions_pareto1 <- apply(pareto_set_E_ALL, 1, dispersion_objective, x = kitchen_dists)
...
dispersions_pareto2 <- apply(pareto_set_VANILLA, 1, dispersion_objective, x = kitchen_dists)

tmp_dists <- as.matrix(kitchen_dists)
...
dispersions_pareto3 <- apply(pareto_set_E_RESTRICTED, 1, dispersion_objective, x = kitchen_dists)
...
worst_cases <- subdf[t(positions_by_value(kitchen_dists, value = target_dispersion)), ] # seems a good value
worst_cases$Match <- 1:2
worst_cases[, c("Item", "Match")] # not at all similar to each other

dispersion_objective(kitchen_dists, selected_partition) # minimum
