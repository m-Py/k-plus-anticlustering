...
BILS_E_1_ILS <- function(data, init_partition, RUNS_MBPI) {
  PARTITIONS <- bicriterion_anticlustering(
    data, 
    K = init_partition, 
    R = c(RUNS_MBPI, RUNS_MBPI)
...
BILS_E_ALL_ILS <- function(data, init_partitions) {
  half_runs <- nrow(init_partitions)/2
  PARTITIONS <- bicriterion_anticlustering(
    data, 
    K = init_partitions[1, ],
    R = c(half_runs, half_runs),
...
BILS_E_ALL_RESTRICTED_ILS <- function(data, init_partitions, cannot_link) {
  distances <- as.matrix(dist(data))
...
# this one uses one half for MBPI, other half for ILS
LCW_RESTRICTED_ILS <- function(data, K, RUNS, cannot_link) {
