...
abstract: |
    ...
    When assigning stimuli to sets in experimental psychology, researchers strive for high similarity between stimulus sets and high pairwise dissimilarity of stimuli within sets. Because stimulus assignment is usually substandard when done manually by researchers, automated algorithmic approaches are preferred to accomplish this goal. Brusco et al. (2020) proposed a bicriterion anticlustering algorithm (BILS) that simultaneously maximizes the diversity (corresponding to between-set similarity) and the dispersion (corresponding to pairwise stimulus dissimilarity within-sets). We provide a first independent assessment of BILS and present BILS-E, which extends BILS in three ways. First, a globally optimal value of dispersion is always returned. Second, the dispersion and the diversity can be optimized on the basis of different dissimilarity matrices, for example to include cannot-link constraints in the anticlustering process. Third, BILS-E allows that other anticlustering objectives of between-group similarity, such as k-means or k-plus, can be optimized instead of only the diversity. Using our optimal approach as benchmark in a simulation study, we show that the original BILS identifies optimal dispersion values for much larger data sets than previously analyzed, but also highlight conditions where it fails to do so. A practical example shows how BILS-E can be used for stimulus assignment in experimental psychology. The original BILS as well as our extension are both freely available via the open source R package anticlust.
