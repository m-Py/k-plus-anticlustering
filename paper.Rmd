...
attributes across the observations belonging the *k*'th cluster ($k = 1, \ldots, 
K$). The centroid of the *k*'th cluster has the notation 
$\overline{\mathbf{x}}^{(k)}$ with
$$
\overline{\mathbf{x}}^{(k)} = 
  (\overline{x}_1^{(k)}, \overline{x}_2^{(k)} \ldots, \overline{x}_P^{(k)})
$$

The mean $\overline{x}_j ^{(k)}$ of the *j*th variable on the *k*th cluster is 
computed as:
...
Based on the original data and the cluster centroids, the SSE---i.e., the k-means 
...
straight-forward extension of the SSE criterion is formulated that can be used 
to minimize differences with regard to the variances of the input variables at 
the same time.

For the sake of simplicity, we first consider a set of $N$ one-dimensional 
observations $(x_1, \ldots, x_N)$. The extension to the general case of $P$ 
attributes will be straight forward, because the formulation of the new 
criterion effectively reduces to the generation of new variables, leaving the 
k-means criterion SSE per sé unchanged. 

The sample variance of this set of 
observations is computed as the mean of the squared distances between 
observations and their mean $\mathbf{x}$:
...
Note at this point that the sample variance is basically defined as a mean 
value. We can use this insight and employ k-means anticlustering to equalize 
this *mean* across groups. For a given $K$, groups can be created that are 
similar with regard to the means of $y$---i.e., the variance of $x$---by 
maximizing the SSE, in this one-dimensional case given as: 
...
When defining $z = (x, y)$, the SSE reduces to the standard case in (X): 
