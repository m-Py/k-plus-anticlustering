...
Note that $\mathit{SSE_{Var}}$ has the same form as the standard $\mathit{SSE}$. However, the input data reflects the squared difference between each data point and the attribute mean instead of the raw data points. Maximizing $\mathit{SSE_{Var}}$ can be used to create groups that have similar variances across the $P$ attributes. As a caveat, it is crucial to note that maximizing $\mathit{SSE_{Var}}$ only reliably yields similar group variances if the group centroids $\overline{\mathbf{x}}^{(k)}$ ($k = 1, \dots, K$) are close to the overall data centroid $\overline{\mathbf{x}}$. This is because the within-group variance is calculated on the basis of the group mean $\overline{\mathbf{x}}^{(k)}$, but the additional variables computed to minimize differences with regard to the variance across groups are calculated using the overall mean $\overline{\mathbf{x}}$. Thus, $\mathit{SSE_{Var}}$ is only valid if (approximately) $\overline{\mathbf{x}}$ = $\overline{\mathbf{x}}^{(k)}$; otherwise we may introduce non-negligible measurement error when optimizing $\mathit{SSE_{Var}}$. Thus, only considering the new variables $\mathbf{Y}_{N \times P}$ in order to minimize discrepancies in variance may not yield satisfying results; instead, it has to be ensured that the group means of the original variables are also similar between groups. Fortunately, minimizing discrepancy with regard to group means can be accomplished by standard k-means anticlustering, i.e., the maximization of $\mathit{SSE}$. Combining the maximization of $\mathit{SSE_{Var}}$ and $\mathit{SSE}$ at the same time leads to the k-plus approach for anticlustering, which is detailed in the next section. 

## A bicriterion model: K-plus anticlustering {#kplus}

An exclusive maximization of $\mathit{SSE_{Var}}$ (i.e., equalizing variances) is not feasible---and most likely not interesting on its own---without also considering the standard $\mathit{SSE}$ (i.e., equalizing means). The most simple approach for an optimization that considers multiple criteria at the same time is the weighted sum approach [@naidu2014; @marler2010]. That is, all criteria are computed independently from each other and are then combined into a single criterion through a weighted sum; a suitable optimization algorithm is then used to maximize the combined criterion. By combining $\mathit{SSE_{Var}}$ and $\mathit{SSE}$ in a single criterion, we define the objective function for k-plus anticlustering:
...
if (!file.exists(path)) {
  ...
  # investigate the best 1% of k-means anticlustering partitions, for discussion
  best_1percent_anticlusterings <- df$kmeans /max(df$kmeans) > .99
  best_partitions <- partitions[best_1percent_anticlusterings]
...
Here, $w_t$ is the weight associated with the $t$'th moment and $\overline{\mathbf{X}}^{(k)}_{T \times P} = \{\overline{x}_{tj}^{(k)}\}_{T \times P}$ is a centroid matrix containing the mean values for all $t$-moment variables in the $k$'th group, i.e.
...
Thus, when setting the weights for all moments to 1, the k-plus criterion $\mathit{SSE_{kplusGeneralized}}$ has the same form as the standard k-means criterion $\mathit{SSE}$, but it is computed on different data. Computing the k-plus criterion is then done by first creating the additional $t$-moment variables, appending them as new columns to the original data matrix, and computing the $\mathit{SSE}$ on the augmented data. Using the unweighted sum approach, k-plus anticlustering therefore leaves the original k-means criterion unchanged.
...
As an additional outlook, it should be noted that because the covariance is defined as an expected value, the k-plus logic can be extended to minimize differences with regard to covariances, a possibility that was not investigated in the current paper. In this case, an additional variable has to be computed for each pair of variables $x = (x_1, \dots, x_N)$ and $y = (y_1, \dots, y_N)$, computed as $(x_i - \overline{\mathbf{x}}) (y_i -\overline{\mathbf{y}})$. Future research should examine how the k-plus approach can be adapted to including covariances. The unweighted standardization approach presented here may serve as a starting point for investigation.
...
Traditionally, anticlustering is implemented by reversing an objective function used in classical cluster analysis. Whereas cluster analysis seeks homogeneity within groups and separation between groups, this reversal leads to heterogeneity within groups and similarity between groups. While tightly linked in a mathematical sense, an important semantic difference is that cluster analysis is concerned with finding "true" structure in the data e.g., by defining a taxonomy on naturally occurring groups [@everitt1979]. Anticlustering instead imposes structure on the data, while the anticlusters have no underlying "true" meaning. An interesting side effect is that we are usually not concerned with the question of whether pairs of elements are assigned to the same anticluster or not. In cluster analysis, similar elements tend to be grouped in the same cluster, and in general this result will be observed across a variety of clustering approaches. In contrast, different anticlustering partitions tend to look very different. In Figure 1, for example, an astonishing `r mean(df$kmeans / max(df$kmeans) >.99) * 100`% of all partitions are within 1% of the global maximum in $\mathit{SSE}$ (i.e., the best k-means anticlustering solution). Among these `r sum(df$kmeans / max(df$kmeans) >.99)` partitions, every possible pair of elements is sometimes in the same group and sometimes not; the minimum amount of co-occurrence is 131 and the maximum amount of co-occurrence is 292. Thus, a high anticlustering objective does not depend on assigning individual items to the same or a different group. In contrast, there are only `r sum(min(df$kmeans)/ df$kmeans > .90)` partitions (`r mean(min(df$kmeans)/ df$kmeans > .90)*100`%) that approximate the global minimum in $\mathit{SSE}$ (i.e., the best k-means clustering solution) by even 10%. Thus, while there are many different high quality anticlustering partitions, there are only very few good clustering partitions, constituting another relevant difference between cluster analysis and anticlustering, despite them being directly and reversely related. Interestingly, the k-plus objective further loosens the link between cluster analysis and anticlustering. Reversing the k-plus anticlustering objective no longer yields a useful clustering interpretation, because identifying groups that are different with regard to higher order moments does not constitute a clustering in the usual sense. Thus, even though k-plus extends the classical k-means clustering criterion, it is a pure anticlustering objective.

An interesting mixture of cluster analysis and anticlustering is fair clustering [e.g. @chhabra2021; @delBarrio2022]. Fair clustering performs a cluster analysis under the restriction that the distribution of a set of "protected" attributes---such as race, sex or age---should be similar between clusters. Thus, as compared to pure cluster analysis, fair clustering biases the identification of groups with regard to the protected attributes, striving for algorithmic fairness. However, a relevant difference to anticlustering is that fair clustering is still concerned with identifying "true" structure in the data [@delBarrio2022], while anticlustering is not concerned with identifying the underlying structure of a data set. However, these methods are similar because anticlustering logically corresponds to fair clustering when only the protected attributes are considered. Hence, a fruitful outlook for research might be to investigate if approaches stemming from the literature on fair clustering can be adopted for anticlustering, and vice versa.
