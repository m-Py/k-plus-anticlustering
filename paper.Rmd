...
Note that $\mathit{SSE_{Var}}$ has the same form as the standard $\mathit{SSE}$. However, the input data reflects the squared difference between each data point and the attribute mean instead of the raw data points. Maximizing $\mathit{SSE_{Var}}$ will lead to similar variances across the $K$ groups in the $P$ attributes (**only if the group means are close to the overall mean, discuss this as suggested in reviewer 1's comment 4**; $\mathit{SSE_{Var}}$ is only valid if the group means are close to the overall means of the data.).

## A multicriterion optimization model: K-plus anticlustering {#kplus}

As discussed (**TODO**) in the previous section, an exclusive maximization of $\mathit{SSE_{Var}}$ (i.e., equalizing variances) is not feasible without also considering the standard $\mathit{SSE}$ (i.e., equalizing means). Moreover, only equalizing variances between groups is most likely not even of interest in most applications. Hence, when striving for similarity in group variances by maximizing $\mathit{SSE_{Var}}$, it is at the same time necessary and worthwhile to strive for similarity in means, thus to maximize $\mathit{SSE}$ at the same time. Hence, a combination of $\mathit{SSE_{Var}}$ and $\mathit{SSE}$ should be employed to simultaneously optimize both criteria, which also represents a reasonable idea when striving for overall between-group similarity. The most simple approach for such a multicriterion optimization is the weighted sum approach [@naidu2014; @marler2010]. That is, all criteria are computed independently from each other and are then combined into a single criterion through a weighted sum
...
w_2 \sum\limits_{j=1}^{P} 
  ...
  (y_{ij} - \overline{y}_j ^{(k)})^2
$$
where the weights $w_1$ and $w_2$ can be chosen to adjust the relative importance of equalizing means and variances, respectively. Maximizing $\mathit{SSE_{kplus}}$ for a given $K$ will lead to similar means as well as variances between groups. 
...
Note that the k-plus logic to equalizing group variances as well as group means can be seamlessly generalized to higher order moments. The *t*'th sample moment of a variable $x = (x_1, \ldots, x_N)$ can be computed as 

$$
\frac{1}{N} \sum\limits_{i=1}^{N} (x_i - \overline{\mathbf{x}})^t.
$$

where $t = 2$ gives the variance, $t = 3$ the skewness, and $t = 4$ the kurtosis [@joanes1998]. Thus, by changing $t$---the power of the deviation between mean and data points---we can use the k-plus logic to equalize any desired sample moment between groups. 

Next, I provide some notation needed to compute the general $\mathit{SSE}$ for k-plus anticlustering, which may include arbitrary distribution moments as part of the computation. For each moment $t$ and for column of the original data matrix $x_{.j} =  (x_{1j}, \ldots, x_{Nj})$ ($j = 1, \ldots, P$) with mean $\overline{\mathbf{x}}_j$, a new variable $x_{t.j} = (x_{t1j}, \ldots, x_{tNj})$ is computed where 
...
This computation results in an augmented data matrix $\mathbf{\hat{X}}_{N \times (T \cdot P)} = \{x_{tij}\}_{N \times (T \cdot P)}$ where $T$ is the number of distribution moments that should be equalized between groups. Note that when defining $x_{tij}$, the condition $t = 1$ is included to keep the unchanged original data as the first $P$ columns in the data, which is used to equalize the the mean---the first moment---between groups. Using the thus defined data matrix $\mathbf{\hat{X}}_{N \times (T \cdot P)}$, the generalized multicriterion for k-plus anticlustering can be formalized by the formula
...
\mathit{SSE_{kplusGeneralized}} =
  ...
  \sum\limits_{k=1}^{K} 
  \sum\limits_{i \in C_k}
  (x_{tij} - \overline{x}_{.j} ^{(k)})^2
  \right)
$$

where $w_t$ is the weight associated with the $t'th$ moment and $\overline{x}_{.j} ^{(k)}$ is the mean of the $j'th$ variable in the $k'th$ group. Maximizing $\mathit{SSE_{kplusGeneralized}}$ with a given number of moments $T$ can be used to equalize these distribution moments between $K$ groups. Note that according to this definition the inclusion of a distribution moment $t$ always implies that all moments $z < t$ are also included as part of the criterion $\mathit{SSE_{kplusGeneralized}}$; it is not deemed worthwhile to exclude lower order moments in favor of higher order moments. In particular, the first moment, represented by the original data, should always be included because otherwise the criterion $\mathit{SSE_{kplusGeneralized}}$ is not valid for higher order moments. 
