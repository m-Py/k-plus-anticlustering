...
w_2 \sum\limits_{j=1}^{P} 
  ...
  (y_{ij} - \overline{y}_j ^{(k)})^2
$$
where the weights $w_1$ and $w_2$ can be chosen to adjust the relative importance of equalizing means and variances, respectively. 
...
where $w_t$ is the weight associated with the $t'th$ moment and $\overline{x}_{.j} ^{(k)}$ is the mean of the $j'th$ variable in the $k'th$ group. Maximizing $\mathit{SSE_{kplus}}$ with a given number of moments $T$ can be used to equalize these distribution moments between $K$ groups. Note that according to this definition the inclusion of a distribution moment $t$ always implies that all moments $z < t$ are also included as part of the criterion $\mathit{SSE_{kplus}}$; it is not deemed worthwhile to exclude lower order moments in favor of higher order moments. In particular, the first moment, represented by the original data, should always be included because otherwise the criterion $\mathit{SSE_{kplus}}$ is not valid for higher order moments. 
