...
(2006) provided in his comprehensive synthesis of the literature 
...
within-group variance. The SSE is the sum of the squared Euclidean distances 
between each data point and its cluster centroid. The cluster centroid is given 
by averaging the values on each variable over the elements within that cluster, 
where the centroid $\overline{x}_j ^{(k)}$ of the *j*th variable in the *k*th cluster is 

$$
\overline{x}_j ^{(k)} = \frac{1}{n_k} \sum\limits_{i \in C_k} x_{ij}
$$

The SSE is then given as:
...
similarity of the mean values of the attributes across the different clusters. 

## K-Variances

K-means anticlustering, i.e., maximizing SSE minimizes differences between the 
means of the input variables. I present a rather simple extension of the SSE 
criterion to simultaneously minimize differences with regard to the variances 
of the input variables. The variance is defined as the sum of the squared 
