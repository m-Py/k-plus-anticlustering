...
Some clarifications with respect to nomenclature are needed. The k-plus criterion is not a single objective function for anticlustering, but instead a familiy of objectives that extend the standard k-means objective ($SSE$) to minimize differences between groups with regard to several characterizations of a data set. That is: not just the mean, but also the variance, covariances, or higher order moments such as skewness and kurtosis. Which of these objectives are pursued depends on a researchers needs. In the most basic---and arguably most important---case, k-plus anticlustering can be used to not just minimize differences with regard to the mean of variables but also their variance. Measures of location (mean) and spread (variance / standard deviation) are arguably most important to researchers when they "eyeball" their data and they are routinely reported as characterizations of a data set. Hence, by default, the k-plus anticlustering objective implies that means and variances should be equal between groups. 
