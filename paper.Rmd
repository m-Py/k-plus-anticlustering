---
title             : "A bicriterion extension of k-means anticlustering"
...
(2006) provided in his comprehensive synthesis of half a century of research on 
k-means. K-means clustering is used to partition two-way, two-mode 
data. That is, $N$ elements each having $P$ attributes into $K$ groups 
$(C_1., \ldots, C_K)$, $C_k$ being the set of $n_k$ objects in group 
...
clustering aims to minimize the error sum of squares (SSE), also known as the 
within-group variance [@spath1986]. The SSE is the sum of the squared Euclidean 
distances between each data point and its cluster centroid. The *k*'th centroid 
is a vector of length $P$, where each entry is the mean value of one of the $P$ 
attributes, computed across the observations belonging the *k*'th cluster ($k = 
1, \ldots, K$). The centroid of the *k*'th cluster $\overline{\mathbf{x}}^{(k)} 
= (\overline{x}_1^{(k)}, \overline{x}_2^{(k)} \ldots, \overline{x}_P^{(k)})$ is 
computed as:
...
Based on the data matrix $\mathbf{X}_{N \times P} = \{x_{ij}\}_{N \times P}$, 
a partitioning of the data into $K$ clusters, the cluster centroids, the 
SSE---i.e., the k-means objective function---is then given as:
...
similarity of the mean values of the $P$ attributes across the $K$ clusters. 

## Bi-criterion extension

K-means anticlustering---i.e., maximizing the SSE---minimizes differences 
between the means of the input variables. In the following, a rather 
straight-forward bicriterion extension of the SSE criterion is formulated that 
can be used to minimize differences with regard to the variances of the input 
variables at the same time.

For the sake of simplicity, we first consider a set of one-dimensional 
observations $(x_1, \ldots, x_N)$ to motivate the new criterion. The extension 
to the general case of $P$ attributes will be straight forward, because the 
formulation effectively reduces to the generation of new variables, whereas the 
original k-means criterion remains unchanged.
...
We can use the insight that the sample variance is basically defined as an 
average, implying that k-means anticlustering can be employed to equalize this 
average across different groups. This is the central insight leading to the new 
criterion. In the one-dimensional case as discussed here, the SSE that 
represents discrepancies with regard to the variance, is given as: 
...
This criterion, reflecting discrepancies in variance across groups, is extended 
to the general case of $P$ attributes as
...
## Bicriterion formulation

It will be rarely of interest to maximize $SSE_{Var}$ (equalizing variances) 
without also considering the standard $SSE$ (i.e., equalizing means). A 
combination of $SSE_{Var}$ and $SSE$, however, can be employed to simultaneously 
optimize both objectives. That is, formulating a criterion incorporating both 
discrepancies in means as well as variances to minimize difference with regard 
to either. The most simple approach to such a bicriterion optimization is the 
weighted sum approach. That is, 

Combining (X) and (Y), we can obtain a combined criterion formulation as a 
weighted sum criterion.

$$
\mathit{SSE_{bicriterion}} = 
  w_1
  \sum\limits_{j=1}^{P} 
  \sum\limits_{k=1}^{K} 
  \sum\limits_{i \in C_k}
...
equalizing means and variances, respectively.

## Unweighted case

Some simplifications can be arranged when ignoring the weights on the two 
criteria. 
...
When defining $z_i = (z_{i1}, z_{i2})$ with $z_{i1} = x_i$ and $z_{i2} = y_i$, 
the bicriterion SSE is computed as
...
This logic can be extended to the general case of $P$ attributes; for each 
attribute, a new variable is created as described above, and the entirety of 
all $2P$ variables is then used as input to standard k-means anticlustering.
...
n_partitions(14, 2)` possible ways to partition the 14 data points into 
...
```{r, fig.cap = "Illustrates the 1-to-1 relationship between SSE and difference in means (Panel A) and variance (Panel B). Each data point illustrates a partition, i.e., a way to divide a data set ($N = 14$) into two groups. In Panel C, the relationship between the two criteria across partitions is plotted. The highlighted symbols indicate membership of the pareto set. The partition represented by the diamond minimizes the difference in variances between groups; the partition represented by the square minimizes the difference in means between groups; the partition represented by the triangular minimizes the sum of the differences in means and variances. ", fig.height = 7, fig.width = 6}

# Plot relationship between objectives
layout(matrix(c(1,2,3,3), 2, 2, byrow = TRUE), heights = c(1, 1.4))
plot(
  df[, c("diff_mean", "kmeans")],
...
legend("bottomleft", legend = "A", cex = 1.3, bty = "n")
plot(
  df[, c("diff_var", "kvar")],
  xlab = "var(f1) - var(f2)",
  ylab = "SSE_var",
...
legend("bottomleft", legend = "B", cex = 1.3, bty = "n")

par(mar = c(7, 10, 0.4, 8) + 0.1)
...
legend("bottomleft", legend = "C", cex = 1.3, bty = "n")
