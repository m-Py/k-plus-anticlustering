...
In this paper, I presented the k-plus criterion for anticlustering, which can be used to divided a data set into subgroups in such a way that the groups are similar to each other. The k-plus criterion extends the classical k-means criterion, which only represents how similar groups are with regard to their means. The k-plus criterion also equalizes the variance between groups, and if needed any higher order moments such as skewness and kurtosis, or even the covariances. Thus, k-plus represents a family of anticlustering criteria, which can be combined by users according to their needs. Interestingly, all k-plus variations basically do not change the original k-means criterion, because additional criteria can be incorporated by augmenting the input data.

A simulation study and examples on real norming data showed that the k-plus criterion is well suited to maximize between-group similarity with regard to multiple criteria. In particular, this work indicates that k-plus anticlustering should be a replacement of the classical k-means approach when striving for between-group similarity.[^binarydata]  K-means anticlustering disregards any distribution characteristics other than the mean, while k-plus anticlustering ensures between-group similarity with regard to multiple criteria---and still minimizes differences in means quite well. The results showed that fulfilling several criteria at the same time may be aggregated in small data sets. Still, users can try out different objectives and see which specification fulfils their needs. A reasonable approach is to start greedy by requesting that multiple distribution moments should equalized. If the results of the greedy setup are subpar, users can reduce their requirements by opting for fewer criteria. Unlike in statistical analysis, there is no problem with "just trying out" different angles when it comes to anticlustering. 

[^binarydata]: A reasonable exception is given when the input data consists of binary variables because in this case the mean and the standard deviation are directly related.

## Limitations and outlook

This paper focused on the presentation of new objective function but the did not really consider the multicriterion nature of the k-plus objective. Instead, a simple (unweighted) sum approach---combined with a generic local maximum neighbourhood search---was used to optimize the k-plus objective. Other approaches, such as direct algorithms, may yield improved results and better opportunities to select among partitionings according to multiple differently weighted k-plus criteria. @brusco2019 showed am example of how direct algorithms can be used to optimize multiple criteria in anticlustering applications. Their study focused on finding the pareto set of solutions that simultaneously optimize diversity and dispersion.[^inanticlust] Future research should also investigate how k-plus anticlustering may profit from more sophisticated algorithmic treatment.
