...
abstract: |
    ...
    Anticlustering refers to a collection of methods that partition data sets into disjunct groups. The partitioning process may pursue two goals: (a) high between-group similarity and (b) high within-group heterogeneity. Many anticlustering objectives simultaneously pursue both goals while other methods focus on a particular aspect. Recently, Brusco, Cradit and Steinley (2020) presented a highly effective bicriterion optimization algorithm that focuses primarily on maximzing within-group heterogeneity. The present paper introduces a novel bicriterion objective for optimizing between-group similarity. *K-plus anticlustering* is a straightforward extension of k-means anticlustering, which maximizes the sum of squared Euclidean distances between each data point and the center of the cluster to which it belongs. Thereby, k-means anticlustering directly minimizes the difference between the mean values of numeric attributes across clusters, but neglects optimizing similarity with regard to the variance of the data sets. K-plus anticlustering extends the k-means objective by adding a term that aims to simultaneously optimize similarity with regard to the variance. Surprisingly, it is shown that the combined objective (i.e., the bicriterion) can be reduced to an augmentation of the input data, leaving the original k-means criterion unchanged. Tests on simulated and real data show that k-plus anticlustering achieves high similarity both with regard to mean attribute values as well as the variance, usually with no noticable cost to either criterion. It is therefore concluded that the extended model should be preferred to classical k-means anticlustering when optimizing between-group similarity.
...
Note that a problem may arise when ignoring the weights during optimizing the
