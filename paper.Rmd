...
In general, the weighting scheme for k-plus anticlustering can become highly complex as it might encompass an arbitrary number of distribution moments, and hence, an arbitrary number of weights. While an exhaustive evaluation of the influence of weights is beyond the scope of the current paper, a small scale simulation study was conducted to investigate the influence of weights for a selected number of moments. The target was the optimization of $\mathit{SSE_{kplusGeneralized}}$ with $T = 4$, i.e. minizimizing differences with regard to means, variances, skew and kurtosis. Two unweighted approaches (using standardization vs. not using standardization) were implemented with $\mathbf{w} = (1, 1, 1, 1)$. The unweighted standardization approach implemented a $z$-standardization of all columns---the original as well as the augmented variables---of $\mathbf{\hat{X}'}_{N \times \mathit{TP}}$ before the criterion $\mathit{SSE_{kplusGeneralized}}$ was optimized. In the other case, the unstandardized matrix $\mathbf{\hat{X}'}_{N \times \mathit{TP}}$ was used. The four weighted approaches were implemented by giving a tenfold increased weight to only one of the four moments. That is, to compute $\mathit{SSE_{kplusGeneralized}}$ favoring similarity in means during the optimization process, I set $\mathbf{w} = (10, 1, 1, 1)$; the weights favoring similarity in variances, skewness and kurtosis, were defined as $\mathbf{w} = (1, 10, 1, 1)$, $\mathbf{w} = (1, 1, 10, 1)$, and $\mathbf{w} = (1, 1, 1, 10)$, repectively. The weighted approaches did not use standardization of the input data.
...
Table 1 illustrates how well each of the six weighting schemes fulfilled each of the four constititung criteria, averaged across the `r length(results)` simulation runs. It is shown that for the weighted approaches, the criterion that received the highest weight was fulfilled to the highest degree, as should be expected. Interestingly, the unweighted approach including standardization had the best average performance across the four criteria. While the advantage over the other weighting schemes does not seem to be remarkable, this pattern was observed in an astonishing `r sum(unstd_best)` of all `r length(results)` simulation runs (`r mean(unstd_best) * 100`%). Among the competing weighting schemes, the unweighted standardization approach therefore seems preferable. It also yielded the best results with regard to maximizing the standard $\mathit{SSE}$. The unweighted standardization approach seemed to implicitly give high priority to minimizing differences in means. This is a pleasant result because, as discussed earlier, for the higher order moment $\mathit{SSE}$s to be valid, the group means have to be close to the overall mean, which is best accomplished if the standard $\mathit{SSE}$ has high priority. 
