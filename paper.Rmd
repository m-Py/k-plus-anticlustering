...
The present paper focused on the new k-plus criterion for anticlustering, but only superficially considered the multicriterion nature of this criterion: A simple unweighted sum approach---combined with a generic local maximum neighbourhood search---was used to optimize the k-plus objective. More sophisticated multicriterion approaches may yield improved results, as well as the opportunity to deliberately select among partitionings that may be preferred according to different k-plus objectives. @brusco2019 showed how a direct algorithm can be used to optimize multiple criteria at the same time in anticlustering applications. Their study focused on approximating the pareto set of efficient solutions that optimize diversity and dispersion, which is essential in applications that require high within-group heterogeneity. Future research should investigate if k-plus anticlustering may profit from similar advanced algorithmic treatment.
