...
The outlined logic to equalize variance in different groups is easily generalized to higher order moments such as skewness and kurtosis, i.e., the third and fourth distribution moments, respectively. Generally, the *j*'th sample moment of a variable $x = (x_1, \ldots, x_N)$ is computed as [@joanes1998]

$$
\frac{1}{N} \sum\limits_{i=1}^{N} (x_i - \overline{\mathbf{x}})^j
$$

where $j = 2$ gives the variance, $j = 3$ the skewness, and $j = 4$ the kurtosis. 

The approach outlined in sections 1.1 and 1.2 illustrated how the definition of the variance can be used to add variables to the data set to minimize differences with regard to the variance between groups. Changing the power of the deviation between mean and data points, we can use the same logic to equalize any desired sample moment between groups. I expect this to be interesting in particular for the skewness and kurtosis to minimize differences with regard to distribution asymmetry (skewness) and the propensity to include outliers (kurtosis). 
...
## The k-plus family of anticlustering objectives

Some clarifications with respect to nomenclature are needed. The k-plus criterion is not a single objective function for anticlustering, but instead is a familiy of objectives that use the standard k-means criterion $SSE$ in order to minimize differences between groups with regard to additional characterizations of a data set; that is, not just the mean, but also the variance, covariances, or higher order moments such as skewness and kurtosis. Which of these objectives are pursued depends on a researchers needs. In the most basic---and arguably most important---case, k-plus can be used to not just minimize differences with regard to the mean of variables but also their variance. Measures of location (mean) and spread (variance / standard deviation) are arguably most important to researchers when they "eyeball" their data and they are routinely reported as characterizations of a data set. Hence, by default, the k-plus anticlustering objective implies that means and variances should be equal between groups. 

While less routinely reported (and less well understood), skewness are kurtosis are also important descriptive characterizations of a data set [@decarlo1997; @westfall2014]. To optimize overall similarity between groups, it may thus be desirable to also minimize differences with regard to these distribution moments. In principle, any higher order moments can also be included as part of the k-plus objective. Note, however, that the inclusion each additional moment requires appending as many features to the data set as there were to begin with. 

K-plus anticlustering can also be used to obtain similar covariance structures among groups. This approach may be of interest in cross validation applications where prediction accuracy depends on the correlations among features and criteria [@zeng2000; @papenberg2020]. Note that each additional objective that is pursued with k-plus anticlustering comes with a cost. It becomes more difficult to satisfactorily fulfil many criteria at the same time. As the simulation study below will show, including many k-plus objectives at the same time becomes more appropriate when the data set becomes larger. In the end, it is a tradeoff between fulfilling few objectives to a precise degree or fulfilling many objectives to a less compelling degree. 

# Simulation study

To systematically evaluate the new k-plus criterion, I compared three anticlustering approaches in a simulation study: (a) k-means anticlustering, optimizing the $\mathit{SSE}$; (b) k-plus anticlustering, optimizing the $SSE_{kplus}$; (c) anticluster editing, optimizing the diversity criterion. All methods used a $z$-standardization of the data sets as a preprocessing step. In the case of k-plus anticlustering, this means that the augmented input data (i.e., the data table that contains the original data and the squared distance data) were subjected to a standardization, which is recommended.
