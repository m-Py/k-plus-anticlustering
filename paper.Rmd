...
abstract: |
    ...
    Anticlustering refers to a collection of methods that partition data sets into disjoint groups while aiming for high between-group similarity or high within-group heterogeneity. Anticlustering is usually approached by maximzing instead of minimizing a clustering objective such as the sum of squared errors used in k-means clustering. Introducing k-plus, this paper presents an extension of the k-means criterion to maximize between-group similarity. While k-means anticlustering minimizes between-group-differences only with regard to the mean of attribute values, k-plus anticlustering can be used to minimize differences with regard to multiple distribution characteristics: the variance, higher order moments such as skewness and kurtosis, and even to preserve covariance structure. Thus, k-plus corresponds to a family of new anticlustering criteria that can be combined in arbitrary ways. It is shown that all k-plus variations can be reduced to an augmentation of the input data while the original k-means objective remains unchanged. A computer simulation and practical examples show that k-plus anticlustering achieves high between-group similarity with regard to multiple criteria with little sacrifice to each single criterion. The k-plus extension is therefore preferred over classical k-means anticlustering when optimizing between-group similarity. Example are given on how k-plus anticlustering can be applied to real norming data using the open source R package `anticlust`, which is freely available via CRAN (https://cran.r-project.org/package=anticlust) and Github (https://github.com/m-Py/anticlust).
