...
This computation results in an augmented data matrix $\mathbf{\hat{X}}_{N \times \mathit{TP}} = \{\hat{x}_{tij}\}_{N \times \mathit{TP}}$, i.e.
...
Note that when defining $\hat{x}_{tij}$, the condition $t = 1$ is separately included to keep the unchanged original data as the first $P$ columns in the data, which is used to equalize the the mean---the first moment---between groups. Using the data matrix $\mathbf{\hat{X}}_{N \times \mathit{TP}}$ and a vector of weights $\mathbf{w} = (w_1, \dots, w_T$), the generalized multicriterion for k-plus anticlustering $\mathit{SSE_{kplusGeneralized}}$ can be formalized:
