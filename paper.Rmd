...
# mean(filter(rel, N > 100)$rel) * 100
  
```

First, I investigated how well maximizing the k-plus criterion approximates the ordinary k-means criterion at the same time. Figure 2 illustrates the relative values of $\mathit{SSE}$ that were found when optimizing $\mathit{SSE_{kplus}}$ as compared to the direct optimization of $\mathit{SSE}$. Generally, optimizing $\mathit{SSE_{kplus}}$ yields an adequate approximation of $\mathit{SSE}$. However, the quality of this approximation depends on the group sizes of the partition problem: For larger $N$, k-plus anticlustering very closely follows k-means anticlustering's performance with regard to optimizing $\mathit{SSE}$. For $N > 100$, $\mathit{SSE}$ was approximated to more than 99.99% when conducting k-plus anticlustering as compared to k-means anticlustering. The minimum approximation for N = 24 and K = 4 was ...
