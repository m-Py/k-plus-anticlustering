...
[^dissimilarityfromfeatures]: It is still possible to convert the attribute data into a dissimilarity matrix, e.g. by computing all pairwise Euclidean distances, and then optimize a distance-based anticlustering criterion to obtain a partitioning.

To optimize overall between-group similarity---and hence to overcome the limitations of k-means anticlustering---this paper introduces k-plus anticlustering. K-plus is an extension of the k-means objective that quantifies between-group similarity as discrepancy with regard to several distribution characteristics instead of only the mean. Specifically, k-plus offers the possibility to minimize differences with regard to the variance and higher order moments such as skewness and kurtosis. After formally introducing the k-plus objective, a simulation study shows that k-plus anticlustering is well-suited to create groups having minimal differences according to multiple distribution characteristics, significantly outperforming traditional anticlustering criteria. Practical examples illustrate how readers can easily apply k-plus anticlustering using the free and open source software package `anticlust` [@papenberg2020; @R-anticlust], an extension to the popular statistical programming language R [@R-base].
...
The *k*'th centroid $\overline{\mathbf{x}}^{(k)} = (\overline{x}_1^{(k)}, \overline{x}_2^{(k)} \ldots, \overline{x}_P^{(k)})$ is a vector of length $P$, where each entry is the mean value of one of the $P$ attributes, computed across all $n_k$ observations belonging to the $k$'th cluster $C_k$ ($k = 1, \ldots, K$):
...
Note that $\mathit{SSE_{Var}}$ has the same form as the standard $\mathit{SSE}$. However, the input data reflects the squared difference between each data point and the attribute mean instead of the raw data points. Maximizing $\mathit{SSE_{Var}}$ can be used to create groups that have similar variances across the $P$ attributes. As a caveat, it is crucial to note that maximizing $\mathit{SSE_{Var}}$ only reliably yields similar group variances if the group centroids $\overline{\mathbf{x}}^{(k)}$, ($k = 1, \dots, K$) are close to the overall data centroid $\overline{\mathbf{x}}$. To reiterate, this condition has to be met for the original data $\mathbf{X}_{N \times P}$. Thus, only considering $\mathbf{Y}_{N \times P}$ in order to minimize discrepancies in variance may not yield satisfying results; instead, it has to be ensured that the group means of the original data are also similar between groups. Fortunately, minimizing discrepancy with regard to group means can be accomplished by standard k-means anticlustering, i.e., the maximization of $\mathit{SSE}$. Combining the maximization of $\mathit{SSE_{Var}}$ and $\mathit{SSE}$ at the same time leads to the k-plus approach for anticlustering, which is detailed in the next section. 

## A multicriterion optimization model: K-plus anticlustering {#kplus}

As discussed in the previous section, an exclusive maximization of $\mathit{SSE_{Var}}$ (i.e., equalizing variances) is not feasible without also considering the standard $\mathit{SSE}$ (i.e., equalizing means). Moreover, maximizing only $\mathit{SSE_{Var}}$ is most likely not interesting in any real application. Hence, when striving for similarity in group variances by maximizing $\mathit{SSE_{Var}}$, it is at the same time necessary and worthwhile to strive for similarity in means, and thus to maximize $\mathit{SSE}$ at the same time. The most simple approach for a multicriterion optimization is the weighted sum approach [@naidu2014; @marler2010]. That is, all criteria are computed independently from each other and are then combined into a single criterion through a weighted sum; a suitable optimization algorithm is then used to maximize the combined criterion. By combining $\mathit{SSE_{Var}}$ and $\mathit{SSE}$ in a single criterion, we define the objective function for k-plus anticlustering: 
...
To formalize the computation of the k-plus criterion for an arbitraty number of distribution moments, some new notation has to be provided. First, we define the $j$'th column ($j = 1, \ldots, P$) of the original data matrix $\mathbf{X}_{N \times P}$ as $x_{.j} =  (x_{1j}, \ldots, x_{Nj})$ where the column mean is $\overline{\mathbf{x}}_j$. Let $T$ be the number of distribution moments that should be equalized between $K$ groups. For each moment $t$ ($t = 1, \ldots, T$) and for each variable $x_{.j}$, a new *t-moment variable* $\hat{x}_{t.j} = (\hat{x}_{t1j}, \ldots, \hat{x}_{tNj})$ is defined where
...
This computation results in a three dimensional tensor $\mathbf{\hat{X}}_{T \times N \times P}$ = $\{\hat{x}_{tij}\}_{T \times N \times P}$ where---in comparison to the original data matrix $\mathbf{X}_{N \times P}$---the additional dimension is given by the $T$ distribution moments that are considered in the anticlustering process. When defining $\hat{x}_{tij}$, the condition $t = 1$ is separately included to keep the unchanged original data as first entry in the "moment dimension" in $\mathbf{\hat{X}}_{T \times N \times P}$, which is used to equalize the the mean---the first moment---between groups. 

Using the tensor  $\mathbf{\hat{X}}_{T \times N \times P}$ and a vector of weights $\mathbf{w} = (w_1, \dots, w_T$), the generalized multicriterion for k-plus anticlustering $\mathit{SSE_{kplusGeneralized}}$ can be formalized:
...
\mathit{SSE_{kplusGeneralized}} =
  ...
  \sum\limits_{k=1}^{K} 
  \sum\limits_{i \in C_k}
  (x_{tij} - \overline{x}_{tj} ^{(k)})^2
  \right)
$$

Here, $w_t$ is the weight associated with the $t$'th moment and $\overline{x}_{tj} ^{(k)}$ is mean for the $k$'th group on the $j$'th $t$-moment variable. Maximizing $\mathit{SSE_{kplusGeneralized}}$ can be used to equalize the first $T$ distribution moments between $K$ groups. 

## The unweighted sum approach for k-plus anticlustering

Some simplifications in the computation of $\mathit{SSE_{kplusGeneralized}}$ can be arranged when ignoring the moment weights $\mathbf{w}$, e.g. by setting $w_t = 1$ ($\forall t = 1, \dots, T$). This simplification leads to the *unweighted sum approach* for k-plus anticlustering. In this case, the additional dimension in $\mathbf{\hat{X}}_{T \times N \times P}$, representing distribution moments can be dropped; instead, the tensor is flattened into a two dimensional matrix, where the variables representing the additional distribution moments are appended to the original data matrix $\mathbf{X}_{N \times P}$, resulting in the augmented matrix $\mathbf{\hat{X}'}_{N \times \mathit{TP}}$:

$$
  \mathbf{\hat{X}'}_{N \times \mathit{TP}} =
...
When defining the row vectors of $\mathbf{\hat{X}'}_{N \times \mathit{TP}}$ with the notation $\hat{x}'_{i.} = (\hat{x}'_{i1}, \ldots, \hat{x}'_{i(TP)})$ for each $i = 1, \ldots, N$, the generalized criterion $\mathit{SSE_{kplusGeneralized}}$ can be computed as
...
\mathit{SSE_{kplusGeneralized}} =
  ...
  \sum\limits_{k=1}^{K} 
  \sum\limits_{i \in C_k}
  (\hat{x}'_{ij} - \overline{\hat{x}'}_j ^{(k)})^2.
$$

Thus, when setting all the weights for all moments to 1, the k-plus criterion $\mathit{SSE_{kplusGeneralized}}$ again has the same form as the standard k-means criterion $\mathit{SSE}$, but it is computed on different data. Here, computing the k-plus criterion is done by first computing the additional $t$-moment variables, appending them as new columns to the original data matrix, and then computing $\mathit{SSE}$ on the augmented data. Using the unweighted sum approach, k-plus anticlustering therefore leaves the original k-means criterion unchanged.

Note that a problem may arise if the weights $\mathbf{w}$ are ignored when computing. Because the computation of the new $t$-moment variables involves squaring, their range of values is expected to be very different from the raw values $x_{ij}$ (e.g., see Figure 1). This imbalance can strongly and unpredictably influence the relative weight of $\mathit{SSE}$ and $\mathit{SSE_{Var}}$ when computing $\mathit{SSE_{kplus}}$. To ensure an equal weight of both criteria, all attributes---i.e., the original data as well as the new $t$-moment varibles---can be standardized before computing $\mathit{SSE_{kplusGeneralized}}$, e.g. via $z$-standardization.

# Evaluation

Two simulation studies were conducted to evaluate the new k-plus objective for anticlustering. A first small scale simulation compared different approaches for weight selection for the k-plus criterion $\mathit{SSE_{kplusGeneralized}}$. In a second large scale simulation study, the k-plus objective was compared to other popular anticlustering objectives, i.e., the k-means and diversity objectives. 
...
note <- paste0("The results of Simulation 1 averaged across all ", length(results), " data sets. Cells indicate how well the maximization of the weighted $\\mathit{SSE_{kplusGeneralized}}$ fulfilled each of the four constituting criteria $\\mathit{SSE}$, $\\mathit{SSE_{Var}}$, $\\mathit{SSE_{Skew}}$, and $\\mathit{SSE_{Kurtosis}}$. Cell values represent relative performance, i.e. they were computed as the $\\mathit{SSE}$ that was attained by the given weighting scheme, divided by the best $\\mathit{SSE}$ that was obtained by any weighting scheme in a given simulation run.")

apa_table(
  final_tab_sim1, 
  escape = FALSE, 
  caption = "Results of Simulation study 1",
  note = note,
...
Table 1 illustrates how well each of the six weighting schemes fulfilled each of the four constititung criteria, averaged across the `r length(results)` simulation runs. It is shown that for the weighted approaches, the criterion that received the highest weight was fulfilled to the highest degree, as should be expected. Interestingly, the unweighted approach including standardization had the best average performance across the four criteria. While the advantage over the other weighting schemes does not seem to be remarkable, this result held across an astonishing `r sum(unstd_best)` simulation runs (`r mean(unstd_best) * 100`%). Among the competing methods, the unweighted standardization approach therefore seems preferable. It is also a strong approach because it yielded the best results with regard to maximizing the standard $\mathit{SSE}$. Hence, the unweighted standardization approach implicitly gives high precedence to minimizing differences in means. This is a pleasant result because, as discussed earlier, for the higher order moment $\mathit{SSE}$s to be valid, it has to be ensured that the group means are close to the overall mean, which is best accomplished if the standard $\mathit{SSE}$ receives high priority. 

## Simulation 2: Comparing k-plus anticlustering to other objectives

To thoroughly evaluate the k-plus approach to anticlustering, two specific k-plus criteria were compared to traditional anticlustering criteria in a second simulation study: (i) "standard" k-plus anticlustering, minimizing differences with regard to group means and variances (by maximizing $\mathit{SSE_{kplus}}$); (ii) "generalized" k-plus anticlustering, minimizing differences with regard to group means, variances, skewness and kurtosis (by maximizing $\mathit{SSE_{kplusGeneralized}}$ with $T = 4$). To implement the k-plus objectives, the unweighted standardization approach was employed that performed best in Simulation 1. Thus, to optimize a k-plus criterion, the original data set was augmented to incorporate the additional criteria and all variables---the original and the augmented variables---were $z$-standardized. Subsequently, the original k-means criterion $\mathit{SSE}$ was maximized on the basis of the augmented data set. The k-plus methods were compared to (i) k-means anticlustering, minimizing differences with regard to group means, (ii) diversity anticlustering, maximizing the sum of pairwise Euclidean distances within groups, (iii) a random allocation of elements to groups.
...
apa_table(
  tab,
  caption = "Results of Simulation study 2",
...
Figure 2 shows that with increasing $N$, k-means and the k-plus methods converged on the same level with regard to minimizing $\Delta \mathit{M}$. This is an important observation because it shows that k-plus anticlustering is capable of addressing multiple objectives at the same time---especially when $N$ increases. However, while k-plus anticlustering was capable of addressing multiple objectives simultaneously, increasing the number of optimization criteria made it more difficult to fulfill each single criterion: k-plus skew/kurtosis performed worse at minimizing $\Delta \mathit{M}$ and $\Delta \mathit{SD}$ than standard k-plus that only considered the mean and variances. However, as intended, k-plus skew/kurtosis was best at minimizing $\Delta \mathit{Skew}$ and $\Delta \mathit{Kurtosis}$ and at the same time, this k-plus objective maintained a comparably good level of addressing $\Delta \mathit{M}$ and $\Delta \mathit{SD}$, outperforming diversity anticlustering. 
...
It should be noted that because the covariance is defined as an expected value, the k-plus logic can be extended to minimize differences with regard to covariance, a possibility that was not investigated in the current paper. The covariance between two variables $x = (x_1, \ldots, x_N)$ and $y = (y_1, \ldots, y_N)$ is defined as $\frac{1}{N} \sum\limits_{i=1}^{N} (x_i - \overline{\mathbf{x}}) (y_i -\overline{\mathbf{y}})$. Thus, to maximize between-group similarity with regard to covariance structure, it is possible to augment the data set with an additional variable for each pair of variables $x$ and $y$, computed as $(x_i - \overline{\mathbf{x}}) (y_i -\overline{\mathbf{y}})$. Future research should examine how the k-plus approach can be adapted to including covariances, including a discussion of how to select weights for the possibly high number of additional variables. The unweighted standardization approach presented here may serve as a starting point for investigation.
