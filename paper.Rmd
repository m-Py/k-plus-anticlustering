...
In this case, the normalizing factor $\frac{N}{K}$ can be ignored when maximizing $f_1$. Thus, the criterion reduces to the sum of the (squared) distances between elements in the same cluster:

$$
\mathit{SSE_{Diversity}} = \sum\limits_{k = 1}^{K}\sum_{(i<j)\in C_k} d_{ij}
$$

This term corresponds to the standard diversity $f_2$, with the distances $d_{ij}$ representing squared Euclidean distances. We denote $f_4$ as the *average diversity criterion*, which maintain unconditional equivalence between k-means and the diversity.
