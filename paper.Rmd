...
When defining a new variable $y = (y_1, \ldots, y_N)$ as the squared deviation of each observation from the mean $\overline{\mathbf{x}}$, i.e. $y_i = (x_i - \overline{\mathbf{x}})^2$ ($i = 1, \dots, N$), the sample variance of $x$ can be reformulated as the mean of $y$:
...
As a secondary purpose of Simulation 2, I investigated the degree to which partitions identified by the different anticlustering methods showed similarity. Specifically, I investigated whether different anticlustering solutions showed any overlap regarding the placement of individual elements in groups. In cluster analysis, high quality partitions according to different objectives tend to overlap to a large degree, but it is an open question whether this result also holds for different anticlustering partitions. To tackle this issue, I computed the adjusted Rand index (ARI) for each pair of partitions that were returned by the competing methods [@rand1971; @hubert1985]. The ARI is an index of the similarity between two partitions, where an index of 0 indicates no similarity and a value of 1 indicates the highest possible similarity (i.e., when two partitions are the same). The pairwise ARIs were computed for each data set and for each $K$, i.e., 30,000 times. 
...
Table 3 illustrates the degree of overlap between anticlustering partitions that were identified in Simulation 2. Overall, it is shown that there was hardly any similarity between anticlustering partitions according to the different criteria. The average ARI was almost 0 in all cases---indicating no similarity of partitions whatsoever. Moreover, the ARI between two anticlustering partitions hardly differed from the ARI between an anticlustering partition and a random partition. This constitutes an interesting and novel result, illustrating that high between-group similarity can be achieved via very different partitionings.
