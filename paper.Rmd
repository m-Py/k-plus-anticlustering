...
In this paper, I presented k-plus anticlustering, a method to divide a data set into similar subgroups. The k-plus criterion extends the classical k-means criterion by incorporating a term that represents how groups differ with regard to their variances. The combined bicriterion objective could be reduced to an augmentation of the input data; for each input variable, a new variable can be computed as the squared deviation between the original values and the overall mean of the original variable. A simulation study and examples on norming data indicate that optimizing the new k-plus objective is well-suited for minimizing differences with regard to means and variances between groups.

K-plus anticlustering should be the default method as a replacement of k-means anticlustering when it comes to creating similar groups. For small samples sizes (or large $K$), k-plus anticlustering may return slightly less-than-optimal results in the eye of the user, but in this case it is no problem to return to k-means and only focus on the means. There is no anticluster hacking; as opposed to some statistical workflows where "mindlessly" trying different approaches until one approach yiels satisfactory results is bad practice, such a trial-and-error approach is unproblematic for anticlustering. Users may simply choose the results they like best. As a starter, I strongly recommend trying out k-plus anticlustering when creating groups that aim for high between-group similarity.
